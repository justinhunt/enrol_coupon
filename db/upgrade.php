...
function xmldb_enrol_coupon_upgrade($oldversion) {
    ...
    if ($oldversion < 2012101400) {
        ...
        $DB->execute("UPDATE {enrol} SET expirythreshold = 86400 WHERE enrol = 'coupon' AND expirythreshold = 0");
        upgrade_plugin_savepoint(true, 2012101400, 'enrol', 'self');
    }

    if ($oldversion < 2012120600) {
        // Enable new self enrolments everywhere.
        $DB->execute("UPDATE {enrol} SET customint6 = 1 WHERE enrol = 'coupon'");
    ...
    if ($oldversion < 2013112100) {
        ...
        $DB->execute("UPDATE {enrol} SET customint1 = 0 WHERE enrol = 'coupon' AND customint1 IS NULL");
