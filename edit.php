...
require_capability('enrol/coupon:config', $context);

$PAGE->set_url('/enrol/coupon/edit.php', array('courseid'=>$course->id, 'id'=>$instanceid));
...
/** @var enrol_coupon_plugin $plugin */
$plugin = enrol_get_plugin(ENROL_COUPON_ENROLTYPE);

if ($instanceid) {
    $instance = $DB->get_record('enrol', array('courseid'=>$course->id, 'enrol'=>ENROL_COUPON_ENROLTYPE, 'id'=>$instanceid), '*', MUST_EXIST);
...
$mform = new enrol_coupon_edit_form(NULL, array($instance, $plugin, $context));
...
if ($mform->is_cancelled()) {
    ...
    if (!isset($data->{ENROL_COUPON_ALLOW_NEW_ENROLS})) {
        // Add previous value of newenrols if disabled.
        $data->{ENROL_COUPON_ALLOW_NEW_ENROLS} = $instance->{ENROL_COUPON_ALLOW_NEW_ENROLS};
    }
...
$PAGE->set_title(get_string('pluginname', 'enrol_coupon'));

echo $OUTPUT->header();
echo $OUTPUT->heading(get_string('pluginname', 'enrol_coupon'));
$mform->display();
echo $OUTPUT->footer();
